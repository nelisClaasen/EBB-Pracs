...
% Package the input arrays of both transistors into one matrix (Needs to be columns)
...
% estimate nr of poles and zeros
n_poles = 1; 
n_zeros = 0; 

% Estimate the transfer functions independently (SISO) to avoid idss error
% tfest_data(samples, output, input)
sys11 = tfest(tfest_data1(:, 1, 1), n_poles, n_zeros, NaN); % PWM1 to T1
sys21 = tfest(tfest_data1(:, 2, 1), n_poles, n_zeros, NaN); % PWM1 to T2

sys12 = tfest(tfest_data2(:, 1, 1), n_poles, n_zeros, NaN); % PWM2 to T1
sys22 = tfest(tfest_data2(:, 2, 1), n_poles, n_zeros, NaN); % PWM2 to T2

% Combine the individual SISO models back into SIMO systems
simo_transfer1 = [sys11; sys21];
simo_transfer2 = [sys12; sys22];
